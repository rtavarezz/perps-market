// 6.3: circuit breakers and trading limits. pauses trading if things go crazy.
// 6.3 has the params/state structs. 6.4 has the risk check functions below.
...
pub struct RiskParams {
    pub max_price_deviation: Decimal,     // max price move per window (0.15 = 15%)
    pub price_window_ms: i64,             // time window for deviation check
    pub min_price_update_interval_ms: i64,
    pub max_open_interest: Quote,         // hard cap on total OI
    pub max_position_ratio: Decimal,      // max single position as fraction of OI
    pub adl_trigger_ratio: Decimal,       // insurance fund min before ADL kicks in
...
// mutable risk state tracked per market
#[derive(Debug, Clone)]
...
// 6.4: check if a new position is within risk limits
...
// check if insurance fund is healthy enough to keep trading
